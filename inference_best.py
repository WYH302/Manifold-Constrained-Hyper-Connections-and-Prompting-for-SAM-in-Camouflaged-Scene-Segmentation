...
def compute_dice(pred: np.ndarray, gt: np.ndarray, eps=1e-8) -> float:
    
    intersection = (pred * gt).sum()
    union = pred.sum() + gt.sum()
    return (2. * intersection) / (union + eps)

def compute_iou(pred: np.ndarray, gt: np.ndarray, eps=1e-8) -> float:
...
def eval_psnr(loader, model, vlm_model, processor, mamba_model, tokenizer,
              ...
              return_individual_scores=False, rank_metric='dice'):
    ...
    individual_scores = [] if return_individual_scores else None

    with torch.no_grad():
        for step, (image, gt2D, img_1024_ori, img_path, indices) in enumerate(loader):
            ...
            if return_individual_scores:
                for b in range(len(pred_mask)):
                    pred_b = pred_mask[b].squeeze().cpu().numpy()
                    gt_b = gt2D[b].squeeze().cpu().numpy()
                    if rank_metric == 'dice':
                        score = compute_dice(pred_b, gt_b)
                    elif rank_metric == 'iou':
                        score = compute_iou(pred_b, gt_b)
                    else:
                        
                        score = compute_dice(pred_b, gt_b)
                    individual_scores.append((indices[b].item(), score))
    ...
    if return_individual_scores:
        return (sm_val, em_val, wfm_val, MAE, dice_val, iou_val, ber_val), individual_scores
    else:
...
class InferenceDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        return img_tensor, gt_tensor, np.array(img_ori), img_path, index
...
def save_top_k_results(indices, score_dict, dataset, model, vlm_model, processor, mamba_model, tokenizer,
                       device, results_dir, hyper_cond, visualize=True):
    ...
    score_file_path = os.path.join(results_dir, "top_k_scores.txt")
    with open(score_file_path, "w") as f:
        f.write("Rank\tIndex\tFilename\tScore\n")

        model.eval()
        with torch.no_grad():
            for rank, idx in enumerate(indices):
                
                img_tensor, gt_tensor, img_np, img_path, _ = dataset[idx]
                filename = os.path.basename(img_path)
                score = score_dict[idx]

                
                f.write(f"{rank+1}\t{idx}\t{filename}\t{score:.4f}\n")

                
                images = img_tensor.unsqueeze(0).to(device)
                gts = gt_tensor.unsqueeze(0).to(device)
                images_ori = torch.from_numpy(img_np).permute(2, 0, 1).unsqueeze(0).to(device)

                
                vlm_inputs = processor(images_ori, return_tensors="pt").to(device)
                vlm_outputs = vlm_model.generate(**vlm_inputs)
                ...
                pred_np = pred_masks[0].squeeze().cpu().numpy()
                pred_img = Image.fromarray((pred_np * 255).astype(np.uint8))
                pred_path = os.path.join(results_dir, "predictions", f"{base_name}_pred.png")
                pred_img.save(pred_path)

                
                if visualize:
                    gt_np = gts[0].squeeze().cpu().numpy()
                    ...
                    if isinstance(img_np, np.ndarray):
                        ...
                        vis_img = images_ori[0].cpu().numpy()
                        if vis_img.shape[0] == 3:
                            vis_img = vis_img.transpose(1, 2, 0)
                    if vis_img.max() > 1:
                        vis_img = vis_img / 255.0
                    vis_path = os.path.join(results_dir, "visualizations", f"{base_name}_vis.png")
                    visualize_prediction(vis_img, gt_np, pred_np, vis_path)
...
def main():
    ...
    parser.add_argument("--top_k", type=int, default=3,
                        help="Keep top K results with highest scores (0 means save all)")
    parser.add_argument("--rank_metric", type=str, default="dice",
                        choices=["dice", "iou"],
                        help="Metric used for ranking (dice or iou)")
    ...
    has_gt = len(test_dataset.gt_files) > 0
    ...
    if has_gt:
        ...
        if args.top_k > 0:
            metrics, individual_scores = eval_psnr(
                test_dataloader,
                integrated_model,
                vlm_model,
                processor,
            ...
            top_k_indices = [idx for idx, _ in individual_scores[:args.top_k]]
            idx_to_score = dict(individual_scores)
        else:
            metrics = eval_psnr(
                test_dataloader,
                integrated_model,
    ...
    if args.save_results:
        ...
        if has_gt and args.top_k > 0:
            
            save_top_k_results(
                indices=top_k_indices,
                score_dict=idx_to_score,
            ...
            print(f"Saved top {args.top_k} results to: {args.results_dir}")
        else:
            
            integrated_model.eval()
            with torch.no_grad():
                for batch_idx, (images, gts, images_ori, img_paths, indices) in enumerate(tqdm(test_dataloader, desc="Saving All Results")):
            ...
            print(f"All prediction results saved to: {args.results_dir}")
